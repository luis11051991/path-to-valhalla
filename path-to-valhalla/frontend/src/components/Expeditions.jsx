        ...
        <div className="h-full relative overflow-hidden flex flex-col">
            {cooldownSeconds > 0 && (
                <div className="bg-red-900/90 text-white text-center py-2 font-bold uppercase tracking-widest text-xs sticky top-0 z-50 flex items-center justify-center gap-2 shadow-lg border-b border-red-500 animate-in slide-in-from-top">
                    <Clock size={16} className="animate-spin-slow" /> Descansando... {cooldownSeconds}s
                </div>
            )}
            ...
            <div className="flex-1 overflow-hidden relative">
                {view === 'MAP' && (
                    <div className="p-6 h-full overflow-y-auto pb-20 custom-scrollbar">
                        <h2 className="text-3xl font-serif text-amber-500 mb-6 border-b border-amber-900/30 pb-2 flex items-center gap-3"><Sword size={32} /> Mapa de Expediciones</h2>
                                ...
                                return (
                                    <div key={zone.id} onClick={() => handleSelectZone(zone)} className={`relative group border-2 rounded-xl overflow-hidden transition-all h-48 cursor-pointer ${isLocked ? 'border-red-900/30 grayscale opacity-60' : 'border-slate-700 hover:border-amber-500 hover:scale-[1.02] shadow-lg'}`}>
                                        ...
                                        <div className="absolute bottom-0 left-0 p-4">
                                            <h3 className="text-xl font-bold text-white drop-shadow-md flex items-center gap-2">{zone.name} {isLocked && <Lock size={16} className="text-red-500" />}</h3>
                ...
                {view === 'ENEMIES' && selectedZone && (
                    <div className="h-full flex flex-col relative">
                        <div className="absolute inset-0 z-0"><img src={selectedZone.image_url} className="w-full h-full object-cover opacity-30 blur-sm" /><div className="absolute inset-0 bg-slate-950/80" /></div>
                        <div className="relative z-10 p-6 flex flex-col h-full overflow-y-auto custom-scrollbar">
                            <button onClick={() => { setView('MAP'); setSelectedZone(null); }} className="self-start mb-4 flex items-center gap-2 text-slate-400 hover:text-white transition-colors"><ArrowLeft size={20} /> Volver</button>
                            <h2 className="text-3xl font-serif text-amber-500 mb-8 text-center drop-shadow-lg">{selectedZone.name}</h2>
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 px-4 pb-20">
                                {enemies.map((enemy) => (<EnemyCard key={enemy.id} enemy={enemy} user={user} onAttack={() => handleAttack(enemy)} disabled={isBattling || cooldownSeconds > 0} />))}
                            </div>
                        </div>
                    </div>
                )}
                ...
                {view === 'BATTLE' && battleResult && (
                    <BattleModal 
                        result={battleResult} user={user} baseEnemy={currentEnemy} playerImage={getAvatarImage()} playerStats={playerStats} 
...
const EnemyCard = ({ enemy, user, onAttack, disabled }) => {
    ...
    let difficultyColor = levelDiff > 2 ? "text-red-500" : levelDiff >= 0 ? "text-yellow-400" : "text-green-400";
    return (
        <div className={`relative bg-slate-900 border transition-all group overflow-hidden flex flex-col ${enemy.is_boss ? 'border-red-600/60 shadow-[0_0_20px_rgba(220,38,38,0.3)]' : 'border-slate-700 hover:border-amber-500'} rounded-xl`}>
            ...
            <div className="p-4 flex-1 flex flex-col bg-slate-900/90">
                ...
                <div className="text-xs text-slate-500 mb-4 flex justify-between"><span>Nvl {enemy.min_level}-{enemy.max_level}</span><span className={difficultyColor}>{levelDiff > 2 ? 'Mortal' : levelDiff >= 0 ? 'Difícil' : 'Fácil'}</span></div>
                <div className="mt-auto"><button onClick={onAttack} disabled={disabled} className={`w-full py-3 rounded font-bold uppercase text-xs tracking-wider flex items-center justify-center gap-2 transition-all ${disabled ? 'bg-slate-800 text-slate-600 cursor-not-allowed border border-slate-700' : 'bg-gradient-to-r from-amber-700 to-amber-600 text-white shadow-lg hover:scale-[1.02]'}`}>{disabled ? 'Descansando...' : <><Sword size={14} /> Atacar (5E)</>}</button></div>
            </div>
        </div>
    );
};

const BattleModal = ({ result, onClose, user, playerImage, baseEnemy, playerStats }) => {
    ...
    useEffect(() => {
        let idx = 0; const log = result.log || [];
        const interval = setInterval(() => {
            if (idx < log.length) {
                ...
                idx++; if (scrollRef.current) scrollRef.current.scrollTop = scrollRef.current.scrollHeight;
            } else { setIsFinished(true); clearInterval(interval); }
        }, 600);
        return () => clearInterval(interval);
    }, [result]);
            ...
            <div className="h-[45%] bg-[url('/backgrounds/arena_bg.png')] bg-cover bg-center relative flex items-center justify-center border-b-4 border-amber-900 shadow-2xl">
                ...
                <div className="relative z-10 flex items-center gap-4 md:gap-12 w-full max-w-5xl px-4 justify-between">
                    ...
                    <div className="flex items-center gap-4 animate-in slide-in-from-left duration-500">
                        <div className="hidden md:flex flex-col gap-2 text-right bg-black/40 p-3 rounded-lg border-r-2 border-amber-600 backdrop-blur-md shadow-lg">
                        ...
                        <div className="w-36 md:w-56 bg-slate-900 border-2 border-amber-600 rounded-lg shadow-[0_0_40px_rgba(245,158,11,0.3)] overflow-hidden flex flex-col transform hover:scale-105 transition-transform">
                            ...
                            <div className="p-2 bg-slate-950 text-center border-t border-slate-800">
                                ...
                                <div className="w-full h-3 bg-slate-800 rounded-full mt-2 overflow-hidden border border-slate-700 relative"><div className="h-full bg-gradient-to-r from-green-600 to-green-400 transition-all duration-500" style={{ width: `${playerPct}%` }} /></div>
                                <div className="text-[10px] text-slate-400 mt-1 font-mono font-bold">{currentHp.player} HP</div>
                            </div>
                        </div>
                    </div>

                    <div className="text-4xl md:text-7xl font-serif text-white/10 italic font-bold select-none">VS</div>
                    ...
                    <div className="flex items-center gap-4 animate-in slide-in-from-right duration-500">
                        <div className="w-36 md:w-56 bg-slate-900 border-2 border-red-600 rounded-lg shadow-[0_0_40px_rgba(220,38,38,0.3)] overflow-hidden flex flex-col transform hover:scale-105 transition-transform">
                            ...
                            <div className="p-2 bg-slate-950 text-center border-t border-slate-800">
                                ...
                                <div className="w-full h-3 bg-slate-800 rounded-full mt-2 overflow-hidden border border-slate-700 relative"><div className="h-full bg-gradient-to-r from-red-600 to-red-400 transition-all duration-500" style={{ width: `${enemyPct}%` }} /></div>
                                <div className="text-[10px] text-slate-400 mt-1 font-mono font-bold">{currentHp.enemy} HP</div>
                            </div>
                        </div>
                        <div className="hidden md:flex flex-col gap-2 text-left bg-black/40 p-3 rounded-lg border-l-2 border-red-600 backdrop-blur-md shadow-lg">
                ...
                <div ref={scrollRef} className="flex-1 overflow-y-auto p-6 space-y-2 font-mono text-sm max-w-4xl mx-auto w-full">
                    ...
                    {isFinished && (
                        <div className="mt-8 p-6 text-center animate-in zoom-in duration-500 bg-slate-900/50 rounded-xl border border-slate-800 shadow-xl max-w-sm mx-auto">
                            {result.isWin ? <div className="text-green-400 font-bold text-2xl uppercase tracking-widest flex flex-col items-center gap-2"><Trophy size={48} className="text-yellow-400 mb-2 drop-shadow-[0_0_10px_rgba(250,204,21,0.5)]" /> ¡VICTORIA!</div> : <div className="text-red-500 font-bold text-2xl uppercase tracking-widest flex flex-col items-center gap-2"><Skull size={48} className="text-red-600 mb-2 drop-shadow-[0_0_10px_rgba(220,38,38,0.5)]" /> DERROTA</div>}
                        </div>
                    )}
                </div>
                {isFinished && (
                    <div className="p-4 bg-slate-900 border-t border-amber-900/30 shrink-0 flex flex-col items-center animate-in slide-in-from-bottom bg-opacity-90 backdrop-blur">
                        {result.isWin && (<div className="flex flex-wrap gap-2 justify-center mb-4"><RewardBadge label={`${result.rewards.xp} XP`} color="text-purple-300" /><RewardBadge label={`${result.rewards.copper} Cobre`} color="text-yellow-300" />{result.rewards.items?.map((item, i) => (<RewardBadge key={i} label={`${item.qty}x ${item.name}`} color="text-white" icon />))}</div>)}
                        <button onClick={onClose} className="w-full max-w-md py-4 bg-gradient-to-r from-amber-700 to-amber-600 hover:from-amber-600 hover:to-amber-500 text-white font-bold uppercase rounded shadow-[0_0_20px_rgba(245,158,11,0.4)] transition-all transform hover:scale-105">{result.isWin ? 'Recoger Botín' : 'Volver al Mapa'}</button>
                    </div>
                )}
            </div>
        </div>
...
const StatRow = ({ icon, label, value, color = "text-slate-200", align = "right" }) => (<div className={`flex items-center gap-2 text-[10px] md:text-xs ${align === "left" ? "flex-row" : "flex-row-reverse"}`}><span className="text-slate-500">{icon}</span><span className="text-slate-400 uppercase">{label}</span><span className={`font-mono font-bold ${color}`}>{value}</span></div>);
const RewardBadge = ({ label, color, icon }) => (<span className={`bg-slate-800 px-3 py-1.5 rounded text-xs border border-slate-700 ${color} flex items-center gap-2 shadow-sm`}>{icon && <div className="w-2 h-2 rounded-full bg-green-500 animate-pulse" />} {label}</span>);
const getLogStyle = (type) => { switch (type) { case 'round': return "text-slate-500 font-bold text-center border-b border-slate-800/50 mt-4 mb-2 pb-1 text-xs tracking-[0.2em]"; case 'player_atk': return "text-blue-300 bg-blue-900/10 border-l-4 border-blue-500 shadow-sm"; case 'enemy_atk': return "text-red-300 bg-red-900/10 border-l-4 border-red-500 shadow-sm"; case 'info': return "text-yellow-500 italic text-center opacity-80 mt-2 text-xs"; default: return "text-slate-300"; }};
