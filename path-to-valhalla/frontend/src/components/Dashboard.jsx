...
const Dashboard = ({ user, onLogout, isShopOpen, onCloseShop, onUpdateUser }) => {
    ...
    const fetchSkills = () => { fetch(apiUrl('/api/my-skills'), { headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` } }).then(res => res.json()).then(data => { if (data.success) setMySkills(data.skills); }); };
    const fetchPets = () => { fetch(apiUrl('/api/my-pets'), { headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` } }).then(res => res.json()).then(data => { if (data.success) { setMyPets(data.pets); const equipped = data.pets.find(p => p.is_active); if (equipped) setActivePet(equipped); else if (data.pets.length > 0) setActivePet(data.pets[0]); } }); };

    useEffect(() => { if (activeTab === 'skills') fetchSkills(); fetchPets(); }, [activeTab]);
    useEffect(() => { if (showAvatarModal) { fetch(apiUrl(`/api/backgrounds?userId=${user.id}`)).then(res => res.json()).then(data => setBackgroundsList(data)); } }, [showAvatarModal, user.id]);

    const handleEquipPet = async (playerPetId) => { const res = await fetch(apiUrl('/api/equip-pet'), { method: 'POST', headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${localStorage.getItem('token')}` }, body: JSON.stringify({ playerPetId }) }); const data = await res.json(); if (data.success) { fetchPets(); const newlyEquipped = myPets.find(p => p.player_pet_id === playerPetId); if (newlyEquipped) setActivePet({ ...newlyEquipped, is_active: true }); } };
    const handleFeedPet = async (playerPetId) => { const res = await fetch(apiUrl('/api/feed-pet'), { method: 'POST', headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${localStorage.getItem('token')}` }, body: JSON.stringify({ playerPetId }) }); const data = await res.json(); if (data.success) { fetchPets(); if (data.newMoney) onUpdateUser({ ...user, ...data.newMoney }); } else setErrorMsg(data.message); };
    const handleToggleEquip = async (skillId) => { try { const res = await fetch(apiUrl('/api/equip-skill'), { method: 'POST', headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${localStorage.getItem('token')}` }, body: JSON.stringify({ skillId }) }); const data = await res.json(); if (data.success) fetchSkills(); else setErrorMsg(data.message); } catch (error) { setErrorMsg("Error de conexión."); } };
    const handleEquipBg = async (bgId) => { const res = await fetch(apiUrl('/api/equip-background'), { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ userId: user.id, backgroundId: bgId }) }); const data = await res.json(); if (data.success) { setCurrentBgUrl(data.newUrl); onUpdateUser({ ...user, active_background_url: data.newUrl, active_background_id: bgId }); } };
    const handleBuyBgClick = (bgId, price) => { setPendingPurchase({ type: 'bg', id: bgId, price, name: 'Fondo Exclusivo' }); };
    const handleRentBagClick = (bagNum) => { const isActive = isBagUnlocked(bagNum); const action = isActive ? "Extender" : "Alquilar"; setPendingPurchase({ type: 'bag', id: bagNum, price: 50, name: `${action} Mochila ${bagNum} (7 días)` }); };
    const executePurchase = async () => { if (!pendingPurchase) return; let url = '', body = {}; if (pendingPurchase.type === 'bg') { url = apiUrl('/api/buy-background'); body = { userId: user.id, backgroundId: pendingPurchase.id }; } else if (pendingPurchase.type === 'bag') { url = apiUrl('/api/rent-bag'); body = { userId: user.id, bagNumber: pendingPurchase.id }; } const res = await fetch(url, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) }); const data = await res.json(); setPendingPurchase(null); if (data.success) { if (pendingPurchase.type === 'bg') { fetch(apiUrl(`/api/backgrounds?userId=${user.id}`)).then(r => r.json()).then(list => setBackgroundsList(list)); onUpdateUser({ ...user, onix: user.onix - pendingPurchase.price }); } else if (pendingPurchase.type === 'bag') { onUpdateUser(data.user); } } else { setErrorMsg(data.message); } };
    const handleSaveStats = async (newStats, pointsSpent) => { try { const response = await fetch(apiUrl('/api/train-stats'), { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ userId: user.id, newStats, pointsSpent }) }); const data = await response.json(); if (data.success) onUpdateUser(data.user); } catch (e) { console.error(e); } };
    const handleOrganizeInventory = async () => { try { const response = await fetch(apiUrl('/api/inventory/organize'), { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ userId: user.id }) }); const data = await response.json(); if (data.success) { onUpdateUser({ ...user, real_inventory: data.inventory }); } else { setErrorMsg(data.message || "Error al organizar."); } } catch (error) { console.error("Error al organizar inventario:", error); setErrorMsg("Error de conexión al organizar."); } };
    ...
    const derivedStats = useMemo(() => {
        ...
        const maxHp = 100 + (totalStats.constitution * 20);
        ...
        return { maxHp, totalDamageMin, totalDamageMax, defense, critChance, blockChance, healPower, skillPower };
    }, [user.stats, totalBonuses]);

    useEffect(() => { if (derivedStats.maxHp !== user.calculatedMaxHp) { onUpdateUser({ ...user, calculatedMaxHp: derivedStats.maxHp }); } }, [derivedStats.maxHp]);

    const getMaxSlots = () => { if (user.level >= 100) return 5; if (user.level >= 50) return 4; if (user.level >= 10) return 3; return 2; };
    const getFeedCostText = (tier) => { if (tier === 1) return "Recupera 20 Saciedad por 10 Cobre"; if (tier === 2) return "Recupera 20 Saciedad por 5 Plata"; if (tier >= 3) return "Recupera 20 Saciedad por 1 Oro"; return "Alimentar mascota"; };
    const isBagUnlocked = (bagNumber) => { if (bagNumber <= 2) return true; if (bagNumber === 3) return user.level >= 20; if (bagNumber >= 4) return user.rented_bags?.some(b => b.bag_number === bagNumber); return false; };
    ...
    const handleDrop = async (e, destination) => {
        ...
        try {
            const response = await fetch(apiUrl('/api/inventory/move'), { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ userId: user.id, itemId: draggedItem.id, destination: destination }) });
    ...
    const formatCurrency = (totalCopper) => { if (!totalCopper) return <span className="text-orange-700 font-bold">0c</span>; const gold = Math.floor(totalCopper / 10000); const remainderAfterGold = totalCopper % 10000; const silver = Math.floor(remainderAfterGold / 100); const copper = remainderAfterGold % 100; return (<span className="flex items-center gap-1 justify-end">{gold > 0 && <span className="text-yellow-500 font-bold">{gold}g</span>}{silver > 0 && <span className="text-slate-300 font-bold">{silver}s</span>}{copper > 0 && <span className="text-orange-700 font-bold">{copper}c</span>}</span>); };
    const ITEM_ICONS = { 'Sword': Sword, 'Shield': Shield, 'Shirt': Shirt, 'Gem': Gem, 'Crown': Crown, 'Footprints': Footprints, 'Sparkles': Sparkles, 'Hand': Hand, 'Scroll': ScrollText };
    const raceData = useMemo(() => RACES.find(r => r.id === user.race) || RACES[0], [user.race]);
    const getAvatarImage = () => { if (user.class_image) { const dbPath = user.class_image; const genderSuffix = user.gender === 'female' ? '_female' : '_male'; const lastDotIndex = dbPath.lastIndexOf('.'); if (lastDotIndex === -1) return dbPath + genderSuffix + ".png"; const path = dbPath.substring(0, lastDotIndex); const ext = dbPath.substring(lastDotIndex); return `${path}${genderSuffix}${ext}`; } return raceData.images[user.gender || 'male']; };
                        ...
                        <div className="bg-black/50 backdrop-blur-sm border border-slate-700 rounded p-3 text-xs space-y-2">
                            ...
                            <div className="grid grid-cols-2 gap-2 mt-2 pt-1">
                                <div className="bg-slate-900/50 p-1 rounded text-center">
                                    <div className="text-[9px] text-slate-500">CRÍTICO</div>
                                    <div className="text-green-400 font-bold">{derivedStats.critChance.toFixed(1)}%</div>
                                </div>
                                <div className="bg-slate-900/50 p-1 rounded text-center">
                                    <div className="text-[9px] text-slate-500">BLOQUEO</div>
                                    <div className="text-blue-400 font-bold">{derivedStats.blockChance.toFixed(1)}%</div>
                                </div>
                                <div className="bg-slate-900/50 p-1 rounded text-center">
                                    <div className="text-[9px] text-slate-500">CURACIÓN</div>
                                    <div className="text-pink-400 font-bold">+{derivedStats.healPower.toFixed(1)}%</div>
                                </div>
                                <div className="bg-slate-900/50 p-1 rounded text-center">
                                    <div className="text-[9px] text-slate-500">SKILL DMG</div>
                                    <div className="text-purple-400 font-bold">+{derivedStats.skillPower.toFixed(1)}%</div>
                                </div>
                            </div>
                        </div>
                    </div>
