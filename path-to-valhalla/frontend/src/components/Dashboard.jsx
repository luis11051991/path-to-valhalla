...
const Dashboard = ({ user, onLogout, isShopOpen, onCloseShop, onUpdateUser }) => {
    ...
    const fetchSkills = () => { fetch(`http://localhost:3000/api/my-skills`, { headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` } }).then(res => res.json()).then(data => { if (data.success) setMySkills(data.skills); }); };
    const fetchPets = () => { fetch(`http://localhost:3000/api/my-pets`, { headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` } }).then(res => res.json()).then(data => { if (data.success) { setMyPets(data.pets); const equipped = data.pets.find(p => p.is_active); if (equipped) setActivePet(equipped); else if (data.pets.length > 0) setActivePet(data.pets[0]); } }); };
    ...
    const executePurchase = async () => { if (!pendingPurchase) return; let url = '', body = {}; if (pendingPurchase.type === 'bg') { url = 'http://localhost:3000/api/buy-background'; body = { userId: user.id, backgroundId: pendingPurchase.id }; } else if (pendingPurchase.type === 'bag') { url = 'http://localhost:3000/api/rent-bag'; body = { userId: user.id, bagNumber: pendingPurchase.id }; } const res = await fetch(url, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) }); const data = await res.json(); setPendingPurchase(null); if (data.success) { if (pendingPurchase.type === 'bg') { fetch(`http://localhost:3000/api/backgrounds?userId=${user.id}`).then(r => r.json()).then(list => setBackgroundsList(list)); onUpdateUser({ ...user, onix: user.onix - pendingPurchase.price }); } else if (pendingPurchase.type === 'bag') { onUpdateUser(data.user); } } else { setErrorMsg(data.message); } };
    const handleSaveStats = async (newStats, pointsSpent) => { try { const response = await fetch('http://localhost:3000/api/train-stats', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ userId: user.id, newStats, pointsSpent }) }); const data = await response.json(); if (data.success) onUpdateUser(data.user); } catch (e) { console.error(e); } };
