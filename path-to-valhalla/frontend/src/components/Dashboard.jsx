...
const Dashboard = ({ user, onLogout, isShopOpen, onCloseShop, onUpdateUser }) => {
    ...
    // CORRECCIÓN: Si no hay fondo personalizado, usa el fondo base de la RAZA
    ...
    const executePurchase = async () => {
        ...
        let url = '', body = {};
        if (pendingPurchase.type === 'bg') { url = 'http://localhost:3000/api/buy-background'; body = { userId: user.id, backgroundId: pendingPurchase.id }; }
        else if (pendingPurchase.type === 'bag') { url = 'http://localhost:3000/api/rent-bag'; body = { userId: user.id, bagNumber: pendingPurchase.id }; }
    ...
    const renderEquipmentSlot = (DefaultIcon, slotName, className = '', tooltipSide = 'top') => {
        ...
        return (
            <div 
                key={slotName}
                className={`w-12 h-12 lg:w-14 lg:h-14 bg-slate-900/80 border-2 rounded flex items-center justify-center relative group shadow-lg z-20 transition-all
                ${item ? 'border-amber-500 bg-slate-800 cursor-grab active:cursor-grabbing' : 'border-slate-600'} 
                ${className}`}
                ...
                <div className="lg:col-span-5">
                    <div className="bg-black/40 backdrop-blur-md border border-amber-900/30 rounded-lg p-4 h-[700px] relative shadow-2xl flex flex-col items-center">
                        ...
                        <div className="relative w-full h-full max-w-[420px]">
                            ...
                            {/* --- SLOT SUPERIORES (DOBLE ARETE) --- */}
                            <div className="absolute top-4 left-0">{renderEquipmentSlot(Sparkles, "earring_1", "", "right")}</div>
                            <div className="absolute top-4 right-0">{renderEquipmentSlot(Sparkles, "earring_2", "", "left")}</div>
                                ...
                                <div className="grid grid-cols-5 gap-1.5 h-full content-start">
                                    {[...Array(40)].map((_, i) => { 
                                        ...
                                        return (
                                            <div 
                                                key={i} 
                                                className={`aspect-square border rounded-sm flex items-center justify-center cursor-pointer shadow-inner relative group transition-colors
                                                ${item ? 'bg-slate-800 border-amber-600/50 cursor-grab active:cursor-grabbing' : 'bg-slate-800/50 border-slate-700 hover:border-amber-500/30'}`}
                                                onDragOver={handleDragOver}
