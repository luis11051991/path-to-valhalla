...
const Dashboard = ({ user, onLogout, isShopOpen, onCloseShop, onUpdateUser }) => {
    ...
    useEffect(() => {
        if (derivedStats.maxHp !== user.calculatedMaxHp) {
            onUpdateUser({ ...user, calculatedMaxHp: derivedStats.maxHp });
        }
    }, [derivedStats.maxHp]);
    ...
    const findDropDestination = (el) => {
        let node = el;
        while (node) {
            const dropType = node.getAttribute?.('data-drop-type');
            if (dropType) {
                const dropSlot = node.getAttribute('data-drop-slot');
                if (dropType === 'bag') return { type: 'bag', slot: Number(dropSlot) };
                if (dropType === 'equipped') return { type: 'equipped', slot: dropSlot };
            }
    ...
    const processDrop = async (destination) => {
        const item = draggedItem;
        if (!item) return;
        if (destination.type === 'bag' && destination.slot === item.bag_slot) { resetDragState(); return; }
        if (destination.type === 'equipped' && destination.slot === item.equipped_slot) { resetDragState(); return; }
        ...
        try {
            const response = await fetch(apiUrl('/api/inventory/move'), {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ userId: user.id, itemId: item.id, destination })
            });
            const data = await response.json();
            if (data.success) {
                onUpdateUser({ ...user, real_inventory: data.inventory });
    ...
    const handleTouchEnd = (e) => {
        ...
        const destination = target ? findDropDestination(target) : null;
        if (destination) {
            processDrop(destination);
        } else {
            resetDragState();
    ...
    useEffect(() => {
        ...
        // Passive must be false to allow preventDefault on touchmove
    ...
    const TOOLTIP_DIMENSIONS = { width: 260, height: 240 };
    const TOOLTIP_GAP = 6;

    const clamp = (value, min, max) => Math.min(Math.max(value, min), max);

    const pickTooltipSide = (rect, preferredSide = 'top') => {
        const space = {
            left: rect.left,
        ...
        const needsWidth = TOOLTIP_DIMENSIONS.width + TOOLTIP_GAP;
        const needsHeight = TOOLTIP_DIMENSIONS.height + TOOLTIP_GAP;
        ...
        const candidates = preferenceOrder[preferredSide] || ['right', 'left', 'bottom', 'top'];
    ...
    const computeTooltipStyle = (rect, side, pointer) => {
        const safeLeft = (val) => clamp(val, 8, window.innerWidth - TOOLTIP_DIMENSIONS.width - 8);
        const safeTop = (val) => clamp(val, 8, window.innerHeight - TOOLTIP_DIMENSIONS.height - 8);

        const pointerX = pointer?.x ?? (rect.left + (rect.width / 2));
        const pointerY = pointer?.y ?? (rect.top + (rect.height / 2));
        const centeredTop = pointerY - (TOOLTIP_DIMENSIONS.height / 2);
        const centeredLeft = pointerX - (TOOLTIP_DIMENSIONS.width / 2);

        if (side === 'left') {
            return {
                position: 'fixed',
                zIndex: 9999,
                left: safeLeft(rect.left - TOOLTIP_DIMENSIONS.width - TOOLTIP_GAP),
                top: safeTop(centeredTop)
            };
        }
        ...
        if (side === 'right') {
            return {
                position: 'fixed',
                zIndex: 9999,
                left: safeLeft(rect.right + TOOLTIP_GAP),
                top: safeTop(centeredTop)
            };
        }
        ...
        if (side === 'bottom') {
            return {
                position: 'fixed',
                zIndex: 9999,
                left: safeLeft(centeredLeft),
                top: safeTop(pointerY + TOOLTIP_GAP)
            };
        }
        ...
        return {
            ...
            zIndex: 9999,
            left: safeLeft(centeredLeft),
            top: safeTop(pointerY - TOOLTIP_DIMENSIONS.height - TOOLTIP_GAP)
        };
    };
    ...
    const handleMouseEnter = (item, e, preferredSide) => {
        if (!item) return;
        const rect = e.currentTarget.getBoundingClientRect();
        const side = pickTooltipSide(rect, preferredSide);
        const pointer = getPointerFromEvent(e);
        setTooltipData({ item, rect, side, target: e.currentTarget, pointer });
    };
    ...
    const renderEquipmentSlot = (DefaultIcon, slotName, className = '', tooltipSide = 'top') => {
        ...
        return (
            <div
                key={slotName}
                data-drop-type="equipped"
                data-drop-slot={slotName}
                ...
                onTouchStart={(e) => { handleTouchStart(item); handleMouseEnter(item, e, tooltipSide); }}
    ...
    const GlobalTooltip = () => {
        ...
        const { item, rect, side, target, pointer } = tooltipData;
        const liveRect = target?.getBoundingClientRect ? target.getBoundingClientRect() : rect;
        ...
        const style = computeTooltipStyle(liveRect, side, pointer);

        return (
            <div style={style} className="bg-slate-950 border-2 border-amber-600 p-3 rounded shadow-[0_0_20px_rgba(0,0,0,0.8)] min-w-[200px] max-w-[260px] pointer-events-none animate-in fade-in zoom-in-95 duration-100">
                ...
                <div className="relative z-10 grid grid-cols-1 lg:grid-cols-12 gap-4 items-start animate-in fade-in duration-300">
                    <div className="lg:col-span-3 space-y-4">
                        ...
                        <div className="bg-black/50 backdrop-blur-sm border border-slate-700 rounded p-3 text-xs space-y-2">
                            ...
                            <div className="flex justify-between items-center border-b border-white/5 pb-1"><span className="text-slate-400">🛡️ Defensa</span><span className="text-white font-mono">{derivedStats.defense}</span></div>
                            <div className="flex justify-between items-center border-b border-white/5 pb-1"><span className="text-slate-400">❤️ Salud Máx</span><span className="text-red-400 font-mono">{user.current_hp} / {derivedStats.maxHp}</span></div>
                            <div className="grid grid-cols-2 gap-2 mt-2 pt-1">
                                <div className="bg-slate-900/50 p-1 rounded text-center"><div className="text-[9px] text-slate-500">CRÍTICO</div><div className="text-green-400 font-bold">{derivedStats.critChance.toFixed(1)}%</div></div>
                                <div className="bg-slate-900/50 p-1 rounded text-center"><div className="text-[9px] text-slate-500">BLOQUEO</div><div className="text-blue-400 font-bold">{derivedStats.blockChance.toFixed(1)}%</div></div>
                                <div className="bg-slate-900/50 p-1 rounded text-center"><div className="text-[9px] text-slate-500">CURACIÓN</div><div className="text-pink-400 font-bold">+{derivedStats.healPower.toFixed(1)}%</div></div>
                                <div className="bg-slate-900/50 p-1 rounded text-center"><div className="text-[9px] text-slate-500">SKILL DMG</div><div className="text-purple-400 font-bold">+{derivedStats.skillPower.toFixed(1)}%</div></div>
                            </div>
                        </div>
                    </div>
                            ...
                            {[...Array(40)].map((_, i) => {
                                ...
                                return (
                                    <div
                                        ...
                                        onDragOver={handleDragOver}
                                        onDrop={(e) => handleDrop(e, { type: 'bag', slot: realBagSlot })}
                                        draggable={!!item}
                                        onDragStart={(e) => handleDragStart(e, item)}
                                        onDragEnd={handleDragEnd}
                                        onTouchStart={(e) => { handleTouchStart(item); handleMouseEnter(item, e, 'left'); }}
                                        onTouchEnd={handleTouchEnd}
