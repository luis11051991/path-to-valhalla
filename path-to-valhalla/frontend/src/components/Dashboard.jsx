...
const MAX_POSSIBLE_SLOTS = 5; // <--- ¡AQUÍ ESTÁ LA VARIABLE QUE FALTABA!
...
const Dashboard = ({ user, onUpdateUser }) => {
    ...
    const getAllowedSlotsForItem = (item) => {
        ...
        // Mapeo simple por nombre de icono si no hay datos explícitos (SOLO TEXTO)
        const iconMap = {
                ...
                <div className="relative z-10 grid grid-cols-1 lg:grid-cols-12 gap-4 items-start animate-in fade-in duration-300">
                    <div className="lg:col-span-3 space-y-4">
                        ...
                        <div className="bg-black/50 backdrop-blur-sm border border-slate-700 rounded p-3 text-xs space-y-2">
                            ...
                            <div className="flex justify-between items-center border-b border-white/5 pb-1"><span className="text-slate-400 flex items-center gap-2"><img src={STAT_IMAGES.health} className="w-5 h-5"/> Salud Máx</span><span className="text-red-400 font-mono">{user.current_hp} / {derivedStats.maxHp}</span></div>
